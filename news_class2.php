...
<body>
    ...
    if (isset($_GET["index"])) {
        if ($_GET["index"] <= 0) {
            header("Location: /news.php?index=1");
        }
        $index = $_GET["index"];
    } else
        $index = 1;
    ...
    <div class="container">
        ...
        <div class="about_top">
            <div class="about_nav">
                ...
                <a href="/news_web.php">WEB开发</a>
                <a href="/news.php">全部新闻</a>
                <a class="active">《二班大乱斗》</a>
            </div>
        </div>
    </div>
    <!--新闻中心-->
    <div class="news_box">
        <ul class="news_list">
            ...
            $index_before = $index * 3;
            $sql = "SELECT * FROM web_announcement";
            $result = $conn->query($sql);
            $count = 0;
            $count_index=0;
...
EOF;
        ...
        <div class="page_box">
            <?php                    //PHP
            $index_pd_next_number = $index_front + 1;
            $index_pd_next_number_plus = $index_front + 2;
            $sql = "SELECT * FROM web_announcement limit " . $index_pd_next_number . "," . $index_pd_next_number_plus;
            // echo $sql;
            $result = $conn->query($sql);
            $row = $result->fetch_assoc();
            if ($index != 1) {
                echo <<<EOF
                <a href="/news_class2.php?index=$index_minus" class="prev">←</a>
EOF;
            }
            if ($index_minus != 0) {
                echo <<<EOF
                <a href="/news_class2.php?index=$index_minus">$index_minus</a>
EOF;
            }
            echo <<<EOF
...
EOF;
            if (!empty($row)) {
                echo <<<EOF
                <a href="/news_class2.php?index=$index_plus">$index_plus</a>
                <a href="/news_class2.php?index=$index_plus" class="next">→</a>
EOF;
            } else {
                echo <<<EOF
