...
<body>
    ...
    <div class="news_box">
        <ul class="news_list">
            ...
            $sql = "SELECT * FROM web_announcement limit " . $index_front . "," . 3 . ";";
            // echo$sql;
            $result = $conn->query($sql);
...
EOF;
        ...
        <div class="page_box">
            <?php                    //PHP
            $index_pd_next_number = $index*3;
            $index_pd_next_number_plus = $index_front + 4;
            $sql = "SELECT * FROM web_announcement limit " . $index_pd_next_number . ",357753";
            // echo $sql;
            $result = $conn->query($sql);
            $row = $result->fetch_array();
            if ($index != 1) {//输出上一个符号
                echo <<<EOF
                <a href="/news.php?index=$index_minus" class="prev">←</a>
                <a href="/news.php?index=$index_minus">$index_minus</a>
EOF;
            }
            else
            {
                echo <<<EOF
                <a class="prev_lucency">←</a>
EOF;
            }
            echo <<<EOF
            <a class="active">$index</a>
            EOF;//输出当前页数
            if (!empty($row)) {//输出下一个页数
                echo <<<EOF
                <a href="/news.php?index=$index_plus">$index_plus</a>
                <a href="/news.php?index=$index_plus" class="next">→</a>
EOF;
            } else {
                echo <<<EOF
